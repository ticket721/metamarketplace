...
module.exports = {
    seal_erc20_allowance_too_low: async function seal_erc20_allowance_too_low() {
        ...
        await EthersERC20Instance.functions.approve(MetaMarketplace.address, BuyPrice - 1);
        await EthersERC2280Instance.functions.approve(MetaMarketplace.address, BuyPrice - 1);
        ...
        await expect(MetaMarketplace.verifySeal(addr, nums, bdata)).to.eventually.be.rejectedWith('MMv0::verifyERC20Payment | allowance too low');
        return expect(MetaMarketplace.seal(addr, nums, bdata)).to.eventually.be.rejected;
