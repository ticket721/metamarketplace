...
module.exports = {
    daiplusoffer_invalid_signature: async function daiplusoffer_invalid_signature() {
        ...
        await expect(MetaMarketplace.checkDaiPlusOffer(
            ...
            `${seller_signature.hex}${buyer_signature.hex.slice(2)}${dpapprove_signature.hex.slice(4)}`,
            {
                from: AuctionRelayer,
                gasPrice: DaiPlusApprove.txparams.gasPrice,
                gasLimit: Math.floor(DaiPlusApprove.txparams.gasLimit * 1.1)
            }
        )).to.eventually.be.rejectedWith('MMv0::checkDaiPlusOffer | signatures should be 195 bytes long');
        ...
        )).to.eventually.be.rejectedWith('MMv0::sealDaiPlusOffer | signatures should be 195 bytes long');
