...
module.exports = {
    seal_invalid_nums_length: async function seal_invalid_nums_length() {
        ...
        const err_nums = nums.slice(0, 1);

        await expect(MetaMarketplace.verifySeal(addr, err_nums, bdata)).to.eventually.be.rejectedWith('MMv0::verifySeal | invalid nums argument length');
        return expect(MetaMarketplace.seal(addr, err_nums, bdata)).to.eventually.be.rejectedWith('MMv0::seal | invalid nums argument length');
