...
module.exports = {
    daiplusoffer_invalid_buyer_signature: async function daiplusoffer_invalid_buyer_signature() {
        ...
        await expect(MetaMarketplace.checkDaiPlusOffer(
            ...
            `${seller_signature.hex}${seller_signature.hex.slice(2)}${dpapprove_signature.hex.slice(2)}`,
            {
                from: AuctionRelayer,
                gasPrice: DaiPlusApprove.txparams.gasPrice,
                gasLimit: Math.floor(DaiPlusApprove.txparams.gasLimit * 1.1)
            }
        )).to.eventually.be.rejectedWith('MMv0::checkDaiPlusOffer | failed buyer signature check');
        ...
        )).to.eventually.be.rejectedWith('MMv0::sealDaiPlusOffer | failed buyer signature check');
    }
}
