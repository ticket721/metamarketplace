...
module.exports = {
    daioffer_invalid_signature: async function daioffer_invalid_signature() {
        ...
        await expect(MetaMarketplace.checkDaiOffer(
            ...
            `${seller_signature.hex}${buyer_signature.hex.slice(4)}`,
            {
                from: AuctionRelayer
            }
        )).to.eventually.be.rejectedWith('MMv0::checkDaiOffer | signatures should be 130 bytes long');
        ...
        )).to.eventually.be.rejectedWith('MMv0::sealDaiOffer | signatures should be 130 bytes long');
