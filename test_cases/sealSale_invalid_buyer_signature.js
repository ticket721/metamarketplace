...
module.exports = {
    sealSale_invalid_buyer_signature: async function sealSale_invalid_buyer_signature() {
        ...
        const buyer_error_bs = `0x${bs.slice(132, 132 + 130)}${bs.slice(2, 132)}${bs.slice(262, 262 + 130)}`;

        await expect(MetaMarketplace.sealSale(id, uints, addr, buyer_error_bs)).to.eventually.be.rejectedWith('MM::sealSale | invalid buyer signature');
