...
module.exports = {
    sealSale_no_fees: async function sealSale_no_fees() {
        ...
        await Dai.mintApprove(buyer.address, 1000, MetaMarketplace.address, 1000);
        await ERC20.mintApprove(buyer.address, 1000, MetaMarketplace.address, 1000);

        expect((await Dai.balanceOf(buyer.address)).toNumber()).to.equal(1000);
        expect((await ERC20.balanceOf(buyer.address)).toNumber()).to.equal(1000);
        ...
        expect((await Dai.balanceOf(seller.address)).toNumber()).to.equal(1000);
        expect((await ERC20.balanceOf(seller.address)).toNumber()).to.equal(1000);
        expect((await Dai.balanceOf(fee_collector)).toNumber()).to.equal(0);
        expect((await ERC20.balanceOf(fee_collector)).toNumber()).to.equal(0);
