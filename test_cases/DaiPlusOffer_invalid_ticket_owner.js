...
module.exports = {
    daiplusoffer_invalid_ticket_owner: async function daiplusoffer_invalid_ticket_owner() {
        ...
        // 1. Create the Ticket
        await ERC721.mint(RealTicketOwner, SCOPE_INDEX);

        // 2. Create the payment + approve marketplace
        await ERC2280.mint(TicketBuyer, BuyPrice + Reward);

        await expect_map(ERC20, ERC2280, ERC721, [TicketOwner, TicketBuyer, AuctionRelayer, RealTicketOwner],
            [0, 0, 0, 0], // Dai
            [0, BuyPrice + Reward, 0, 0], // Dai+
            [0, 0, 0, 1], // T721
            expect
        );
        ...
        return expect(MetaMarketplace.checkDaiPlusOffer(
        ...
        )).to.eventually.be.rejectedWith('MMv0::checkDaiPlusOffer | seller is not ticket owner');
