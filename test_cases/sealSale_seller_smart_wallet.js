...
module.exports = {
    sealSale_seller_smart_wallet: async function sealSale_seller_smart_wallet() {
        ...
        const sellerInstance = await SmartWalletMockArtifact.new(sellerWallet.address);
        const seller = {
            privateKey: sellerWallet.privateKey,
            address: sellerInstance.address
        };
        ...
        expect((await Dai.balanceOf(fee_collector)).toNumber()).to.equal(100);
        expect((await ERC20.balanceOf(fee_collector)).toNumber()).to.equal(100);
