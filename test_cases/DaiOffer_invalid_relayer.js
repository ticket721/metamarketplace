...
module.exports = {
    daioffer_invalid_relayer: async function daioffer_invalid_relayer() {
        ...
        const WrongAuctionRelayer = accounts[3];
        ...
        await expect(MetaMarketplace.checkDaiOffer(
            [TicketOwner, TicketBuyer, WrongAuctionRelayer],
        ...
        )).to.eventually.be.rejectedWith('MMv0::checkDaiOffer | invalid relayer');

        return expect(MetaMarketplace.sealDaiOffer(
            [TicketOwner, TicketBuyer, WrongAuctionRelayer],
            [DaiOffer.auction.ticket, DaiOffer.auction.nonce, DaiOffer.amount, DaiOffer.reward],
            `${seller_signature.hex}${buyer_signature.hex.slice(2)}`,
            {
        ...
        )).to.eventually.be.rejectedWith('MMv0::relayerCheck | invalid relayer');
