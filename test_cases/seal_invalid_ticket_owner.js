...
module.exports = {
    seal_invalid_ticket_owner: async function seal_invalid_ticket_owner() {
        ...
        // 1. Create the Ticket
        await ERC721.mint(TicketBuyer, SCOPE_INDEX);
        const Ticket = 1;
        ...
        await expect(MetaMarketplace.verifySeal(addr, nums, bdata)).to.eventually.be.rejectedWith('MMv0::verifySeal | seller is not ticket owner');
        return expect(MetaMarketplace.seal(addr, nums, bdata)).to.eventually.be.rejectedWith('MMv0::seal | seller is not ticket owner');
