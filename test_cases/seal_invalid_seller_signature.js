...
module.exports = {
    seal_invalid_seller_signature: async function seal_invalid_seller_signature() {
        ...
        const err_bdata = `0x${bdata.slice(2 + 0, 2 + 130)}${bdata.slice(2 + 0, 2 + 130)}${bdata.slice(2 + 260)}`;

        await expect(MetaMarketplace.verifySeal(addr, nums, err_bdata)).to.eventually.be.rejectedWith('MMv0::verifySeal | invalid seller signature');
        return expect(MetaMarketplace.seal(addr, nums, err_bdata)).to.eventually.be.rejectedWith('MMv0::seal | invalid seller signature');
