...
module.exports = {
    seal_invalid_addr_length: async function seal_invalid_addr_length() {
        ...
        const err_addr = addr.slice(0, 1);

        await expect(MetaMarketplace.verifySeal(err_addr, nums, bdata)).to.eventually.be.rejectedWith('MMv0::verifySeal | invalid address argument length');
        return expect(MetaMarketplace.seal(err_addr, nums, bdata)).to.eventually.be.rejectedWith('MMv0::seal | invalid address argument length');
