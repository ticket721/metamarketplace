...
module.exports = {
    sealSale_invalid_event_controller: async function sealSale_invalid_event_controller() {

        const { accounts, expect, network_id } = this;

        const MetaMarketplace = this.contracts[CONTRACT_NAME];
        const { ERC721, Dai, ERC20, T721ControllerMock } = this.contracts;
        ...
        await T721ControllerMock.setReturnedValue(false);

        await expect(MetaMarketplace.sealSale(id, uints, addr, bs)).to.eventually.be.rejectedWith('MM::sealSale | event controller has no authority upon provided ticket');
