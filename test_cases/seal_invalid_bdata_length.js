...
module.exports = {
    seal_invalid_bdata_length: async function seal_invalid_bdata_length() {
        ...
        const err_bdata = bdata.slice(0, 4);

        await expect(MetaMarketplace.verifySeal(addr, nums, err_bdata)).to.eventually.be.rejectedWith('MMv0::verifySeal | invalid bdata argument length');
        return expect(MetaMarketplace.seal(addr, nums, err_bdata)).to.eventually.be.rejectedWith('MMv0::seal | invalid bdata argument length');
