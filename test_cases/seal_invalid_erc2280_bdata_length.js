...
module.exports = {
    seal_invalid_erc2280_bdata_length: async function seal_invalid_erc2280_bdata_length() {
        ...
        const err_bdata = bdata.slice(0, 2 + 260);

        await expect(MetaMarketplace.verifySeal(addr, nums, err_bdata)).to.eventually.be.rejectedWith('MMv0::verifyERC2280Payment | invalid bdata argument length');
        return expect(MetaMarketplace.seal(addr, nums, err_bdata)).to.eventually.be.rejectedWith('MMv0::processERC2280Payment | invalid bdata argument length');
