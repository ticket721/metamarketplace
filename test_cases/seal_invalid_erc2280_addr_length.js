...
module.exports = {
    seal_invalid_erc2280_addr_length: async function seal_invalid_erc2280_addr_length() {
        ...
        const err_addr = addr.slice(0, 2);

        await expect(MetaMarketplace.verifySeal(err_addr, nums, bdata)).to.eventually.be.rejectedWith('MMv0::verifyERC2280Payment | invalid address argument length');
        return expect(MetaMarketplace.seal(err_addr, nums, bdata)).to.eventually.be.rejectedWith('MMv0::processERC2280Payment | invalid address argument length');
