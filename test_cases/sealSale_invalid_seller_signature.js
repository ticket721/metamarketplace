...
module.exports = {
    sealSale_invalid_seller_signature: async function sealSale_invalid_seller_signature() {
        ...
        const seller_error_bs = `0x${bs.slice(2, 132)}${bs.slice(2, 132)}${bs.slice(262, 262 + 130)}`;

        await expect(MetaMarketplace.sealSale(id, uints, addr, seller_error_bs)).to.eventually.be.rejectedWith('MM::sealSale | invalid seller signature');
