...
module.exports = {
    daiplusoffer_invalid_seller_signature: async function daiplusoffer_invalid_seller_signature() {
        ...
        await expect(MetaMarketplace.checkDaiPlusOffer(
            ...
            `${buyer_signature.hex}${buyer_signature.hex.slice(2)}${dpapprove_signature.hex.slice(2)}`,
            {
                from: AuctionRelayer,
                gasPrice: DaiPlusApprove.txparams.gasPrice,
                gasLimit: Math.floor(DaiPlusApprove.txparams.gasLimit * 1.1)
            }
        )).to.eventually.be.rejectedWith('MMv0::checkDaiPlusOffer | failed seller signature check');
        ...
        )).to.eventually.be.rejectedWith('MMv0::sealDaiPlusOffer | failed seller signature check');
